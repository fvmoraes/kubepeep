...
var redactionRules = []redactionRule{
	...
	{regexp.MustCompile(`(?i)\b(bearer)(\s+)(?:"(?:\\.|[^"\\\r\n])*"|'[^'\r\n]*'|[A-Za-z0-9._~+/=-]{8,})`), `${1}${2}[REDACTED]`},
	{regexp.MustCompile(`(?i)\b(basic)(\s+)(?:"(?:\\.|[^"\\\r\n])*"|'[^'\r\n]*'|[A-Za-z0-9+/=]{8,})`), `${1}${2}[REDACTED]`},
	{regexp.MustCompile(`\b[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{10,}\b`), `[REDACTED_JWT]`},
	{regexp.MustCompile(`(?i)("(?:password|passwd|pwd)"\s*:\s*")[^"]*(")`), `${1}[REDACTED]${2}`},
	{regexp.MustCompile(`(?i)\b(password|passwd|pwd)\s*[:=]\s*[^\s,;]+`), `${1}=[REDACTED]`},
