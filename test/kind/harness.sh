...
benchmark_dataset=$harness_dir/benchmark/dataset.py
...
Commands:
  ...
  benchmark [OUTPUT]    run the representative synthetic baseline (default: .state/)
  benchmark-matrix [OUTPUT]
                        run the complete synthetic dimension matrix (default: .state/)
  benchmark-dataset [OUTPUT] [NAMESPACES] [PODS_PER_NAMESPACE]
                        generate, but never apply, one deterministic Kind dataset
...
generated_target_is_safe() {
	...
	[ ! -L "$output_file" ] && [ -f "$output_file" ] ||
		fail "refusing to replace non-regular benchmark target: $output_file"
	LC_ALL=C grep -Fqm1 "$marker" "$output_file" ||
		fail "refusing to replace a file not generated by the benchmark harness: $output_file"
}
...
run_synthetic_benchmark() (
	...
	trap 'rm -f -- "${temporary_output:-}"' 0 HUP INT TERM
	benchmark_commit=$(git -C "$repo_dir" rev-parse HEAD)
	benchmark_worktree=clean
	[ -z "$(git -C "$repo_dir" status --porcelain)" ] || benchmark_worktree=dirty
	benchmark_status=0
	if (
		cd "$repo_dir"
		go run ./test/kind/benchmark -suite "$suite" -commit "$benchmark_commit" \
			-working-tree "$benchmark_worktree" -output "$temporary_output"
	); then
		benchmark_status=0
	else
		benchmark_status=$?
	...
	temporary_output=
	trap - 0 HUP INT TERM
	[ "$benchmark_status" -eq 0 ] || fail "$suite synthetic benchmark reported unexpected or unstable outcomes; report preserved at $output_file"
	say "$suite synthetic benchmark written to $output_file (working tree: $benchmark_worktree)"
)
...
generate_benchmark_dataset() (
	...
	need python3
	generated_target_is_safe "$output_file" '"app.kubernetes.io/managed-by":"kubepeep-benchmark-lab"'
	output_dir=$(dirname -- "$output_file")
	mkdir -p -- "$output_dir"
	temporary_output=$(mktemp "$output_dir/.kubepeep-dataset.XXXXXX")
	trap 'rm -f -- "${temporary_output:-}"' 0 HUP INT TERM
	python3 "$benchmark_dataset" --namespaces "$namespace_count" \
		--pods-per-namespace "$pods_per_namespace" >"$temporary_output"
	chmod 600 "$temporary_output"
	mv -- "$temporary_output" "$output_file"
	temporary_output=
	trap - 0 HUP INT TERM
	say "dataset manifest written to $output_file; it was not applied"
)
...
static_validate() {
	...
	trap 'rm -f -- "${dataset_probe:-}"' 0 HUP INT TERM
	python3 "$benchmark_dataset" --namespaces 1 --pods-per-namespace 10 >"$dataset_probe"
	python3 - "$fixture_file" "$cluster_file" "$app_e2e_driver" "$benchmark_dataset" "$dataset_probe" "$kind_node_image" <<'PY'
import pathlib
import re
import sys
...
fixture_path, cluster_path, app_e2e_path, benchmark_dataset_path, dataset_probe_path = map(pathlib.Path, sys.argv[1:6])
kind_node_image = sys.argv[6]
...
compile(benchmark_dataset_path.read_text(encoding="utf-8"), str(benchmark_dataset_path), "exec")
benchmark_documents = list(yaml.safe_load_all(dataset_probe_path.read_text(encoding="utf-8")))
if not benchmark_documents or any(not isinstance(document, dict) for document in benchmark_documents):
    raise SystemExit("generated benchmark dataset must contain only non-empty YAML mappings")
benchmark_pods = [document for document in benchmark_documents if document.get("kind") == "Pod"]
if len(benchmark_pods) != 10:
    raise SystemExit("1x10 benchmark probe did not emit exactly ten Pods")
required_profiles = {"global-viewer", "namespace-viewer", "mixed-viewer", "no-watch-viewer"}
actual_profiles = {
    document.get("metadata", {}).get("name")
    for document in benchmark_documents
    if document.get("kind") == "ServiceAccount"
}
if not required_profiles.issubset(actual_profiles):
    raise SystemExit("generated benchmark dataset is missing an RBAC/watch profile")
for index, document in enumerate(benchmark_documents, start=1):
    metadata = document.get("metadata", {})
    if metadata.get("labels", {}).get("app.kubernetes.io/managed-by") != "kubepeep-benchmark-lab":
        raise SystemExit(f"benchmark document {index} has no benchmark ownership label")
    if document.get("kind") in {"Role", "ClusterRole"}:
        for rule in document.get("rules", []):
            if any("*" in rule.get(field, []) for field in ("apiGroups", "resources", "verbs")):
                raise SystemExit(f"benchmark document {index} contains wildcard RBAC")
    if document.get("kind") == "Pod":
        for container in document.get("spec", {}).get("containers", []):
            if immutable_image.fullmatch(container.get("image", "")) is None:
                raise SystemExit(f"benchmark document {index} contains a mutable image")
            security_context = container.get("securityContext", {})
            if (
                security_context.get("allowPrivilegeEscalation") is not False
                or security_context.get("readOnlyRootFilesystem") is not True
                or security_context.get("privileged") is not False
                or security_context.get("capabilities", {}).get("drop") != ["ALL"]
            ):
                raise SystemExit(f"benchmark document {index} has an unsafe container securityContext")
PY
	rm -f -- "$dataset_probe"
	dataset_probe=
	trap - 0 HUP INT TERM
	(
		cd "$repo_dir"
		go test ./test/kind/benchmark
	)
	say "manifests and benchmark dataset parse locally, pin every image digest and contain no cluster-admin/wildcard grants"
}
...
benchmark)
	[ "$#" -le 2 ] || { usage >&2; exit 2; }
	run_synthetic_benchmark representative "${2:-$state_dir/performance-representative.json}"
	;;
benchmark-matrix)
	[ "$#" -le 2 ] || { usage >&2; exit 2; }
	run_synthetic_benchmark matrix "${2:-$state_dir/performance-matrix.json}"
	;;
benchmark-dataset)
	[ "$#" -le 4 ] || { usage >&2; exit 2; }
	dataset_namespaces=${3:-10}
	dataset_pods=${4:-100}
	generate_benchmark_dataset "${2:-$state_dir/dataset-${dataset_namespaces}x${dataset_pods}.yaml}" \
		"$dataset_namespaces" "$dataset_pods"
	;;
refresh-grant)
	[ "$#" -eq 1 ] || { usage >&2; exit 2; }
	guard_cluster
