...
- [ ] **F2-01** Definir personas, problemas resolvidos, objetivos, não objetivos e fronteira exata do MVP.
- [ ] **F2-02** Especificar os fluxos: primeiro uso, seleção de contexto, seleção de escopo, overview, navegação de recursos, logs e ações.
- [ ] **F2-03** Definir estados de carregamento, vazio, offline, proibido, parcialmente disponível e cancelado.
- [ ] **F2-04** Definir quando uma ação fica oculta e quando fica visível porém desabilitada com explicação.
- [ ] **F2-05** Especificar a arquitetura de informação e o menu compacto: Overview, Workloads, Pods, Logs, Events, Network, Config, Namespaces, Permissions e Settings.
- [ ] **F2-06** Documentar tokens visuais Catppuccin Mocha, acento mauve, densidade, tipografia monoespaçada e requisitos de acessibilidade.
- [ ] **F2-07** Definir os critérios de aceite por jornada e ligar cada um à matriz do MVP.
...
- [ ] **F2-14** Especificar watches compartilhados, SSE unidirecional e um transporte bidirecional seguro para exec, condicionado ao spike do Ginger.
- [ ] **F2-15** Definir carregamento progressivo do dashboard e contrato de erros parciais por bloco.
...
- [ ] **F2-21** Definir comportamento fail-closed, erro 403 padronizado e revalidação imediatamente antes de toda ação.
- [ ] **F2-22** Definir confirmação contextual para ações destrutivas e a política de logs operacionais sem conteúdo sensível.
- [ ] **F2-23** Definir política de Secrets: somente metadados; nenhum valor em DTO, YAML, log, fixture ou banco.
...
- [ ] **F2-28** Documentar tudo o que é proibido persistir e como inspecionar o banco nos testes.

### API

- [ ] **F2-29** Classificar rotas como MVP obrigatório, posterior ao MVP ou suporte interno.
- [ ] **F2-30** Definir DTOs de request/response sem expor objetos completos do client-go.
...
- [ ] **F2-37** Transformar cada fase seguinte em tarefas rastreáveis a requisito, endpoint, tela e teste.
- [ ] **F2-38** Definir fixtures sintéticos e a matriz de identidades RBAC para integração/E2E.
- [ ] **F2-39** Definir os comandos oficiais de lint, teste, build, smoke e E2E no `docs/implementation-plan.md`.
- [ ] **F2-40** Revisar todas as especificações em conjunto e resolver contradições antes do gate.
- [ ] **F2-41** Fixar nomenclatura, casing, module path, nomes de archives e URLs de instalação canônicos.
- [ ] **F2-42** Definir o contrato operacional de foreground/daemon, bind por tentativa real, prontidão, single instance, `start`, `stop`, `status`, timeout e sinais por plataforma.
- [ ] **F2-43** Definir allowlist e schema versionado de preferências, filtros salvos e configurações do dashboard.
- [ ] **F2-44** Proibir impersonation, coleta de credenciais adicionais e qualquer autorização fora do Kubernetes.
- [ ] **F2-45** Definir `Cache-Control: no-store` para API Kubernetes/logs/permissões e impedir cache offline/service worker de dados do cluster.
- [ ] **F2-46** Definir OpenTelemetry como opt-in, desativado por padrão e sem dependência obrigatória no core.
- [ ] **F2-47** Planejar um harness Kind/K3d incremental a partir da Fase 4, para não postergar a validação RBAC real até a distribuição.
- [ ] **F2-48** Definir como `resourceName` participa das consultas e chaves de cache de autorização para ações sobre objetos específicos.
- [ ] **F2-49** Definir cursor composto vinculado a query/context generation para fan-out em namespace/kind, merge determinístico, expiração/410 e semântica honesta de busca/ordenação.
- [ ] **F2-50** Fixar o payload de `/health` com aplicação, SQLite, kubeconfig, cluster e contexto, distinguindo falha local crítica de dependência Kubernetes degradada.
- [ ] **F2-51** Fixar o schema observável `timestamp`, `level`, `component`, `operation`, `request_id`, `context`, `namespace`, `resource`, `duration` e `error_code`.
- [ ] **F2-52** Fixar `GET /api/v1/workloads/{kind}/{namespace}/{name}/yaml`, `GET /api/v1/pods/{namespace}/{name}/yaml` e o padrão equivalente para recursos permitidos; Secret não possui rota YAML.
- [ ] **F2-53** Fixar `GET /api/v1/endpoint-slices` e `GET /api/v1/secrets` metadata-only, incluindo paginação e a allowlist exata de campos expostos.
- [ ] **F2-54** Incluir `complete`, `truncated`, cobertura de namespaces e erros parciais em agregações limitadas por budget.
- [ ] **F2-55** Definir limites de bytes por linha, container, stream e scan, além de linhas/pods/concorrência.
- [ ] **F2-56** Definir SQLite por conexão: foreign keys, busy timeout, pool, journal/WAL, lock de instância e inspeção de journal/backup.
- [ ] **F2-57** Definir LIST inicial + resourceVersion, permissão distinta de watch, relist após 410, bookmarks, timeoutSeconds e fallback quando watch for negado.
- [ ] **F2-58** Definir como o conjunto ordenado de arquivos de `KUBECONFIG` é representado/persistido sem armazenar conteúdo ou credenciais.
- [ ] **F2-59** Fixar `GET /api/v1/port-forwards` e `DELETE /api/v1/port-forwards/{id}` para lifecycle das sessões criadas pelo endpoint do Pod.
- [ ] **F2-60** Fixar `GET/PUT /api/v1/preferences` com chaves allowlisted para Settings, filtros e dashboard.
- [ ] **F2-61** Definir checks, formato, códigos de saída e sanitização de `kubePeep doctor`, distinguindo falha local de cluster apenas degradado.
...
- precedência entre flag `--kubeconfig`, `KUBECONFIG` e `~/.kube/config`;
