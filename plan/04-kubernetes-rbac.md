...
- cache concorrente de clients pelo conjunto ordenado de paths + contexto + fingerprints;
...
- [ ] **F4-01** Implementar `KubeconfigLoader` com precedência de flag, `KUBECONFIG` e arquivo padrão definida na especificação.
...
- [ ] **F4-06** Criar cache concorrente pelo conjunto ordenado de arquivos + contexto + fingerprints.
- [ ] **F4-07** Invalidar cache em mudança de qualquer path/contexto, symlink/replace/modificação de arquivo e erro de autenticação classificado.
...
- [ ] **F4-10** Implementar listagem de contextos com DTO mínimo e sem dados de autenticação.
- [ ] **F4-11** Implementar seleção de contexto e atualização transacional do profile padrão.
- [ ] **F4-12** Implementar `GET /api/v1/contexts`, `POST /api/v1/contexts/select` e `GET /api/v1/cluster/profile`.
...
- [ ] **F4-15** Implementar parser canônico de entrada por linhas, vírgulas, ponto e vírgula, espaço não ambíguo, YAML simples e JSON simples.
- [ ] **F4-16** Remover vazios e duplicados, preservar ordem estável e contabilizar separadamente válidos, duplicados descartados e inválidos.
...
- [ ] **F4-19** Implementar modelos e serviços para `single`, `list` e `all`, incluindo namespace padrão consistente.
- [ ] **F4-20** Salvar todos os itens de um escopo em uma única transação e uma única chamada da interface.
- [ ] **F4-21** Implementar `GET /api/v1/namespaces`, `GET/POST /api/v1/namespace-scopes`, `GET/PUT/DELETE /api/v1/namespace-scopes/{id}` e `POST /api/v1/namespace-scopes/validate`.
- [ ] **F4-22** Rejeitar conflito de nome/item com erro útil e preservar o banco se qualquer etapa transacional falhar.
- [ ] **F4-23** Criar interface de escopos com nome, contexto, modo, namespace padrão, textarea, chips, três contadores, validar, salvar, remoção individual e limpeza.
...
- [ ] **F4-32** Representar a chave completa: kubeconfig/contexto, namespace, grupo de API, recurso, subresource, verbo e `resourceName` quando a decisão for específica de um objeto.
- [ ] **F4-33** Criar cache de permissões com TTL entre 30 e 60 segundos e deduplicação de requests concorrentes.
...
- [ ] **F4-38** Implementar `GET /api/v1/permissions` e matriz textual de capabilities na interface.
...
- [ ] **F4-48** Criar o harness mínimo Kind/K3d com recursos permitidos em um namespace, recursos negados em outro e identidade sem `cluster-admin`.
- [ ] **F4-49** Executar no harness os fluxos de contexto, `single`, `list`, `all`, acesso negado e refresh de permissões.
...
- [ ] **F4-51** Conectar `--kubeconfig`, `--context` e `--namespace` ao bootstrap real, profile/escopo ativo e cache de clients; testar precedência, valor inválido e efeito observado.
- [ ] **F4-52** Adicionar e fixar versões coerentes de `k8s.io/client-go`, `k8s.io/api`, `k8s.io/apimachinery` e, somente para a integração opcional, `k8s.io/metrics`; validar version skew e `go list -m`.
- [ ] **F4-53** Criar testes frontend de importação por linhas/vírgulas, duplicados, inválidos, `single`, `list`, `all` e cancelamento ao trocar contexto.
