...
- [ ] **F0-06 — Traces OTel (D04).** Spans de `resources.list` (`.global`/`.fanout`/`.origin`), `resources.merge`, `cursor.get/put`, `watch.connect/reconnect`, caches existentes com atributos seguros e sem alta cardinalidade sensível. Definir o contrato de `cache.*` do novo resource cache; sua integração real e evidência ficam na F2 (C04).
- [ ] **F0-07 — Laboratório de benchmark (D02).** Estender `test/kind/` com geração de datasets (1/10/25/50/100/200 namespaces × 10–500 pods), RBAC variado (global, namespace-only, misto, authorization unavailable, watch on/off), latência simulada (0–200 ms) e injeção de erros (429/410/timeout/reset). Script idempotente e repetível; separar os cenários de limite em C02 e registrar o protocolo de medição de C04.
...
| Cenários de 200 namespaces de C02 | caminho global autorizado sem erro por cursor pesado; caminho restrito preserva teto/rejeições legítimas; over-fetch e memória medidos separadamente |
