...
function LogsWorkspace({ selection, params, defaults, preferencesUnavailable }: { selection: SelectionSummary; params: URLSearchParams; defaults: Preferences['logs']; preferencesUnavailable: boolean }) {
  ...
  return <>
    ...
    <section className="log-controls" aria-label="Log query">
      ...
      <div className="form-actions"><button type="button" className="button" disabled={!ready || read.isPending} onClick={() => read.mutate()}>{read.isPending ? 'Reading…' : 'Read logs'}</button><button type="button" className="button button--secondary" disabled={!ready || previous || isFollowing} onClick={() => void startFollow()}>Follow</button><button type="button" className="button button--danger" disabled={!isFollowing} onClick={() => stopFollow()}>Stop</button></div>
    ...
    <section className="log-viewer">
      <header><div><strong className={`follow-state follow-state--${follow.status}`} aria-live="polite">{follow.message}</strong><small>{visibleLines.length} visible of {pausedLines.length} line{pausedLines.length === 1 ? '' : 's'} kept in the bounded in-memory viewer.</small></div><div><button type="button" className="button button--compact button--secondary" onClick={() => { if (!paused) setPausedAt(lines.length); setPaused(!paused) }}>{paused ? 'Resume view' : 'Pause view'}</button><button type="button" className="button button--compact button--secondary" onClick={() => setWrap(!wrap)}>{wrap ? 'Disable wrap' : 'Wrap lines'}</button><button type="button" className="button button--compact button--secondary" disabled={visibleLines.length === 0} onClick={() => void copyLogs()}>Copy</button><button type="button" className="button button--compact button--secondary" disabled={visibleLines.length === 0} onClick={downloadLogs}>Download</button><button type="button" className="button button--compact button--secondary" disabled={lines.length === 0} onClick={() => { setFollowLines([]); read.reset(); setPaused(false) }}>Clear</button></div></header>
      <label className="log-search">Search visible logs<input value={search} maxLength={256} onChange={(event) => setSearch(event.target.value)} /></label>
      <pre className={wrap ? 'log-output log-output--wrap' : 'log-output'} aria-label="Log output">{visibleLines.map((line, index) => <span key={`${index}-${line.timestamp ?? ''}`}>{timestamps && line.timestamp ? <time>{line.timestamp} </time> : null}{line.text}{line.truncated ? ' [truncated]' : ''}{'\n'}</span>)}</pre>
